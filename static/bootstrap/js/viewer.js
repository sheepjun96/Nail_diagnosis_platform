...
async function loadSeriesListForViewer(searchQuery = "") {
    ...
    const url = `/api/resource/viewer/series/list?stl_seq=${encodeURIComponent(stlSeq)}&search=${encodeURIComponent(searchQuery)}`;
    ...
    try {
        ...
        console.error("Failed to load series via SQL search:", err);
    }
}

function initViewerSearch() {
    const searchInput = document.getElementById("main_search_input");
    const searchBtn = searchInput?.closest('.input-group')?.querySelector('button.btn-accent');

    if (searchInput && searchBtn) {
        searchBtn.onclick = () => {
            loadSeriesListForViewer(searchInput.value.trim());
        };
...
function filterSeriesTable() {
    ...
    for (let i = 0; i < rows.length; i++) {
        const diagCell = rows[i].querySelector(".diagnosis-col");
        
        if (diagCell) {
            const textValue = diagCell.textContent || diagCell.innerText;
